...
const AboutPage = ({ showError }) => {
  const [about, setAbout] = useState({});
  const [es, setEs] = useState(EditorState.createEmpty());

  const getAbout = async () => {
    try {
      const response = await settingsServices.get('nevjegy');
  ...
  const handleSubmit = () => {
    try {
      ...
      showError('success', 'Siker', 'A névjegy sikeresen elmentve.');
    } catch (error) {
      showError('error', 'Rendszerhiba!', 'Kérjük próbálja meg újra pár perc múlva.');
      console.error(error);
    }
  ...
  return (
    <Root max="100%" title="Felhasználási feltételek" onSubmit={handleSubmit} controls={<ReFreshBtn callback={getAbout} />}>
      <Editor
        editorStyle={{
